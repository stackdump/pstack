...
ec_address = os.environ.get(
    "EC_ADDR",
    "EC3Hu1W7uMHf7CtSva1cMyr5rXKsu7rVqQtkJCDHqEV9dgh5FjAj")
...
def create_new_did():
    ...
    # New public and authentication key with specified key type, controller
    # and priority requirement
    ...
    new_did = (
        DID()
        # Can be mainnet/testnet or omitted entirely, in which case no network
        # specifier will be added to the DID
        .testnet()
...
def update_existing_did(did):
    did.update().add_did_key(
        ...
        "my-did-key-5", [DIDKeyPurpose.AuthenticationKey,
                         DIDKeyPurpose.PublicKey]
    ).add_management_key(
...
def encrypt_keys_as_str_and_decrypt():
    ...
    print(
        "{} -> {}".format(first_management_key_alias,
                          first_management_key_private_key)
    )
...
def decrypt_keys_from_file():
    ...
    file_path = os.path.join(
        "examples",
        "paper-did-UTC--2019-09-11T07_42_16.244Z.txt")
    password = "123qweASD!@#"
